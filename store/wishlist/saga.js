...
function* getWishlistListSaga() {
    try {
        const localWishlistList = JSON.parse(
            localStorage.getItem('persist:chickchack')
        ).wishlist;
...
function* addItemToWishlistSaga(payload) {
    try {
        ...
        const { productSelected } = payload;
        let localWishlist = JSON.parse(
            JSON.parse(localStorage.getItem('persist:chickchack')).wishlist
        );

        let existItem = localWishlist.wishlistItems.find(
            (item) => item.productSelected.id === productSelected.id
        ); 

        if (!existItem) {
            localWishlist.wishlistItems.push({product:product, productSelected: productSelected});
